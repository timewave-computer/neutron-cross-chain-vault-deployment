...
use cctp_lend_types::ethereum_config::{
...
struct Vault {
    ...
    max_rate_increment_bps: u32,
    max_rate_decrement_bps: u32,
    starting_rate: U256,
...
struct CctpTransfer {
    ...
    cctp_transfer_threshold_amt: u64,
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let one_way_vault_config = OneWayVaultConfig {
        ...
        maxRateDecrementBps: parameters.vault.max_rate_decrement_bps,
        minRateUpdateDelay: parameters.vault.min_rate_update_delay,
    ...
    let cctp_transfer_config = CCTPTransferConfig {
        amount: U256::ZERO, // Send everything
        mintRecipient: FixedBytes::<32>::from_str(&parameters.cctp_transfer.recipient)?,
        inputAccount: deposit_account,
        destinationDomain: parameters.cctp_transfer.destination_domain,
        cctpTokenMessenger: parameters.cctp_transfer.cctp_token_messenger,
        transferToken: parameters.vault.deposit_token,
    };

    let cctp_transfer = CCTPTransfer::deploy_builder(
        &rp,
        parameters.general.owner,
        processor_address,
        cctp_transfer_config.abi_encode().into(),
    );

    let cctp_transfer: Address = eth_client
        .sign_and_send(cctp_transfer.into_transaction_request())
        .await?
        .contract_address
        .unwrap();
    println!("CCTP Transfer deployed at: {cctp_transfer}");
    ...
    let eth_cfg = EthereumStrategyConfig {
        ibc_transfer_threshold_amt: U256::from(
            parameters.cctp_transfer.cctp_transfer_threshold_amt,
        ),
